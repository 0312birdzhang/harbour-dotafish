...
function loadDotaMetadata() {
	...
	$.ajax({
		type:'GET',
		cache: true,
		url:'http://www.dota2.com/jsfeed/itemdata?v=3840381b3840381&l=english',
		dataType:'json',
...
function notifyIncorrectGuess() {
	...
	if ( g_RemainingGuesses == 0 ) {
		...
		showAlertMessage( "Final Score: " + g_Score, function() {
			resetScore();
			resetPage();
		});
	}
...
function setupPage() {
	...
	if ( g_totalQuestions.length == 0 ) {
		setTimeout( notifyParentOfCompletion, 1000 );
		showAlertMessage( "Completion Score: " + g_Score, function() {
			resetScore();
			resetPage();
		});
